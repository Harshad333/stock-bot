...
class EnhancedBuyerSellerDetectionSensitive:
    def __init__(self):
        ...
        self.buyer_seller_threshold = 0.35  # Was 0.6, now 0.35 (40% lower)
        self.strong_signal_threshold = 0.55  # Was 0.75, now 0.55 (25% lower)
        self.volume_threshold = 1.5  # Was 3.0, now 1.5 (50% lower)
        self.momentum_threshold = 0.15  # Was 0.3, now 0.15 (50% lower)
        self.buying_pressure_threshold = 0.55  # Was 0.65, now 0.55 (15% lower)
        
        # TRADING SETTINGS
        self.ENABLE_TRADING = True  # Set to False for paper trading
        self.ORDER_QUANTITY = 75   # 1.5 Lot Nifty
    ...
    def get_tick_data(self):
        """Get real-time tick data for precise analysis"""
        try:
    ...
    def enhanced_buyer_seller_analysis_sensitive(self, df):
        ...
        # Final decision with peak awareness
        if buyer_score > seller_score and buyer_score > 0.13:  # Lowered from 0.35 to 0.13
            if current_peak:
                direction = 'BUYERS_AT_PEAK'
                confidence = min(buyer_score * 80, 75)  # Reduced confidence at peak
            else:
                direction = 'BUYERS_DOMINATING'
                confidence = min(buyer_score * 100, 95)
        elif seller_score > buyer_score and seller_score > 0.13:  # Lowered from 0.35 to 0.13
            if current_trough:
                direction = 'SELLERS_AT_TROUGH'
                confidence = min(seller_score * 80, 75)  # Reduced confidence at trough
            else:
    ...
    def check_immediate_momentum(self, df):
        ...
        try:
            ...
            # MUCH LOWER thresholds for immediate detection
            if abs(price_change) > 20:  # 20+ points move (was 50+)
                if price_change > 0 and volume_ratio > 1.2:  # Was 2.0, now 1.2
            ...
            # Medium moves (10+ points)
            elif abs(price_change) > 10:  # NEW: 10+ points detection
            ...
            # Small moves (5+ points) - EVEN LOWER threshold
            ...
            # Tiny moves (3+ points) - ULTRA LOW threshold
    ...
    def detect_price_velocity(self, df):
        ...
        try:
            ...
            # Velocity thresholds (ULTRA LOW)
            if velocity > 2:  # 2+ points per minute = BUYERS
                return {'direction': 'BUYERS', 'confidence': min(0.8, 0.5 + abs(velocity) * 0.05)}
            elif velocity < -2:  # -2 points per minute = SELLERS
                return {'direction': 'SELLERS', 'confidence': min(0.8, 0.5 + abs(velocity) * 0.05)}
            elif velocity > 1:  # Mild bullish velocity
                return {'direction': 'BUYERS', 'confidence': 0.6}
            elif velocity < -1:  # Mild bearish velocity
                return {'direction': 'SELLERS', 'confidence': 0.6}
            elif velocity > 0.5:  # Very mild bullish
                return {'direction': 'BUYERS', 'confidence': 0.55}
            elif velocity < -0.5:  # Very mild bearish
                return {'direction': 'SELLERS', 'confidence': 0.55}
    ...
    def place_order(self, symbol, token, quantity, order_type="BUY", entry_price=None):
        """Place Normal Order (Entry only - SL/Target placed separately)"""
        try:
    ...
    def exit_position(self, order_id, reason):
        ...
        try:
            ...
            # Simulate or get actual exit price
            exit_ltp = self.get_option_ltp(symbol) or position['entry_price']
    ...
    def manage_active_positions(self):
        ...
        for order_id, position in list(self.active_positions.items()):
            ...
            current_ltp = self.get_option_ltp(symbol)
        ...
        while True:
            try:
                ...
                if result:
                    ...
                    # 65 points minimum for trade (65% of 100)
                    min_trade_score = 65  # 65% threshold
                    if trade_score['total_score'] >= min_trade_score:
                        ...
                        if len(self.active_positions) == 0:
                            ...
                            # PUT CHECK: SELLERS velocity
                            if velocity_direction == 'SELLERS' and velocity_confidence > 0.5:
                            ...
                            # VELOCITY NOT MET
                            if velocity_confidence <= 0.5 or velocity_direction not in ['BUYERS', 'SELLERS']:
                        ...
                        print(f"   Missing {missing_points:.1f} points for 65% trade signal")
    ...
    def calculate_trade_score(self, result, trend_filter):
        ...
        try:
            ...
            # 1. Price Velocity (40 points - BOOSTED)
            price_velocity_data = result['methods'].get('price_velocity', {'confidence': 0})
            price_velocity = price_velocity_data['confidence']
            ...
            # 3. Buyer/Seller Score (15 points) - LOWERED THRESHOLD
            ...
            # Determine final signal (Total = 100 points)
            if total_score >= 80:  # 80%+
                signal = 'STRONG_TRADE'
            elif total_score >= 65:  # 65%+ (THRESHOLD)
                signal = 'MODERATE_TRADE'
            elif total_score >= 50:  # 50%+
                signal = 'WEAK_SIGNAL'
            else:
                signal = 'NO_TRADE'
    ...
    def generate_trade_recommendation(self, result, trade_score, pivot_points):
        ...
        try:
            ...
            if direction == 'BUY':
                ...
                if contract:
                    opt_ltp = self.get_option_ltp(contract['symbol']) or 0
